            ...
            {data.map((row, index) => {
                ...
                return(
                    <Link key={index} to={row.link} style={{color: '#292929', textDecoration: 'none'}}>
                        <MenuItem key={index} paperprops={{backgroundColor: col}}>
                            {IconComponent}
                            {headerComp}
                            {row.text}
                        </MenuItem>
...
const Header = ({userdata}) => {
    const myAccountlist = [ {text: 'My account', icon: <Avatar/>, link: `profile`}, {text: 'Reset Password', icon: <RotateLeft/>, link: 'profile/password'}, {text: 'Logout', icon: <Logout/>, link: '/'}]
